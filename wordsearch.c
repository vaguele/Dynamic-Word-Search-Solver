...
void findFirst(char **arr, int **pathfinder, char *word, struct tuple *letterBank){
    letterTrack = 0;
    struct tuple first;
    verify = letterBank;
    for(int i = 0; i < bSize; i++){
        for(int j = 0; j < bSize; j++){
            if(*(*(arr + i) +j) == *(word + letterTrack)){
                first.row_focus = i;
                first.col_focus = j;
                *(letterBank + letterTrack) = first;
                letterTrack += 1;
                letterBankLength += 1; // stop directions see line 111
                *(*(pathfinder + i) + j) = letterTrack; 
                return;
            }
        }
    }
...
void directions(char **arr, char *word, struct tuple *letterBank, int letterTrack, int struct_row, int struct_col){
    ...
    if(*(*(arr + struct_row) + struct_col) == *(word + letterTrack)){ 
...
void searchPuzzle(char** arr, char* word) {
    ...
    findFirst(arr, pathfinder, word, letterBank);

    if(letterBankLength == 0){
         printf("\nWord not found!");
         return;
    }

    directions(arr, word, letterBank, letterTrack, verify->row_focus, verify->col_focus);
    printf("\n");
    printf("Word found! \n");
